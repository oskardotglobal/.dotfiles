...
var TagClosing2 = class {
  constructor(client, descriptionLanguageId) {
    ...
    this._cancel = void 0;
    import_coc20.workspace.onDidChangeTextDocument((event) => this.onDidChangeTextDocument(event.textDocument, event.contentChanges), null, this._disposables);
    this.updateEnabledState();
    import_coc20.workspace.registerAutocmd({
      event: ["BufEnter"],
      request: false,
  ...
  async updateEnabledState() {
    ...
    if (!import_coc20.workspace.getConfiguration(void 0, document.uri).get(`${configLang}.autoClosingTags`)) {
      return;
    }
    this._enabled = true;
  }
  ...
  async onDidChangeTextDocument(documentEvent, changes) {
    ...
    const document = await import_coc20.workspace.document;
    if (!document) {
      return;
    }
    ...
    if (!import_coc20.Range.is(lastChange["range"]) || !lastChange.text) {
      return;
    }
    ...
    const priorCharacter = lastChange["range"].start.character > 0 ? activeDocument.getText(import_coc20.Range.create(import_coc20.Position.create(rangeStart.line, rangeStart.character - 1), rangeStart)) : "";
    if (priorCharacter === ">") {
      return;
    }
    this._timeout = setTimeout(async () => {
      ...
      const position = addedLines.length <= 1 ? import_coc20.Position.create(rangeStart.line, rangeStart.character + lastChange.text.length) : import_coc20.Position.create(rangeStart.line + addedLines.length - 1, addedLines[addedLines.length - 1].length);
      const args = Position.toFileLocationRequestArgs(filepath, position);
